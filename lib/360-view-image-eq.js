...
// Start loading the video (if not already playing)
if (video.readyState === 0) {
	video.load(); // Manually trigger loading if needed
} else {
  console.log("Video already loaded, initializing Three.js");
  init();
  renderLoop();
}
...
function init() {
	...
	texture.magFilter = THREE.LinearFilter; // or NearestFilter
	texture.format = THREE.RGBAFormat;  // Adjust format as needed
