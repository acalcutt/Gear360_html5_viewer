...
$geojsonData = array(); // Initialize the array for geoJSON data
...
foreach ($file_list as $file_path) {
    ...
    if (in_array($ext, $allowed)) {
        ...
        if ($gps) {
            ...
            // Create the image_id from the filename without extension
            $image_id = pathinfo($file_path, PATHINFO_FILENAME);

            // Construct the image URL
            $image_url = $root_website . $file_path;  // Assumes images are in a subdir "images"

            // Add the data to the array in the desired format
            $geojsonData[] = array(
                "image_id" => $image_id,
...
$json_output = json_encode($geojsonData, JSON_PRETTY_PRINT | JSON_NUMERIC_CHECK);
