<?php
  ...
  if (isset($_GET['x']) && is_numeric($_GET['x']) && (0 <= $_GET['x']) && ($_GET['x'] <= 360)) {
    $x = (int)$_GET['x'];  // Cast to integer for safety
  }

  // Validate that the y value has a numeric and is inside -180 - 180
  if (isset($_GET['y']) && is_numeric($_GET['y']) && (-180 <= $_GET['y']) && ($_GET['y'] <= 180)) {
    $y = (int)$_GET['y'];
  }

  // Validate that the z value is inside the correct range
  if (isset($_GET['z']) && is_numeric($_GET['z']) && (-180 <= $_GET['z']) && ($_GET['z'] <= 180)) {
    $z = (int)$_GET['z'];
  }

  if (isset($_GET['zoom']) && is_numeric($_GET['zoom']) && (.4 <= $_GET['zoom']) && ($_GET['zoom'] <= 3)) {
    $zoom = (float)$_GET['zoom'];  // Cast to float
  }
